...
acts_as_follower is a gem to allow any model to implement self relation to the records of the same type. For eg: the friendships of a user can be implemented easily by self relation technique of act_as_buddy.

Main uses would be for Users create friends etc..
...
The gem version does not work with rails 2.3.x. Sorry guys its right time you guys think about an upgrade ;)
...
Make your model(s) that you want to allow to be followed act_as_buddy, just add the mixin:
...
Copyright (c) 2013 kannancet(krxcet@gmail.com). See LICENSE.txt for
further details.
